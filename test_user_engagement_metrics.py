...
def patch_globals(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(user_engagement_metrics, "USERNAMES_FILE", str(tmp_path / "usernames.txt"))
    monkeypatch.setattr(user_engagement_metrics, "OUTPUT_FILE", str(tmp_path / "user_results.jsonl"))
    monkeypatch.setattr(user_engagement_metrics, "CHECKPOINT_FILE", str(tmp_path / "completed_usernames.txt"))
    yield

def test_safe_get_rate_limit(monkeypatch):
    m_resp = MagicMock()
    m_resp.status_code = 403
    m_resp.headers = {'X-RateLimit-Remaining': '0', 'X-RateLimit-Reset': str(int(time.time()) + 1)}
...
def test_get_user_profile(monkeypatch):
    monkeypatch.setattr(user_engagement_metrics, "safe_get", lambda url: MagicMock(json=lambda: {"login": "foo"}))
...
def test_get_starred_repos_count_with_link(monkeypatch):
    m_resp = MagicMock()
    m_resp.headers = {'Link': '<https://api.github.com/user/123/starred?page=42>; rel="last"'}
    m_resp.json.return_value = [1]
    monkeypatch.setattr(user_engagement_metrics, "safe_get", lambda *a, **k: m_resp)
    assert user_engagement_metrics.get_starred_repos_count("foo") == 42

def test_get_orgs(monkeypatch):
    monkeypatch.setattr(user_engagement_metrics, "safe_get", lambda url: MagicMock(json=lambda: [{"login": "acme"}]))
...
def test_search_user_contributions_commit(monkeypatch):
    ...
    monkeypatch.setattr(user_engagement_metrics, "safe_get", lambda url, params=None, extra_headers=None: m_resp)
